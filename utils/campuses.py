...
def get_campus(_filter, admin=False):
    campus = app.db.session.query(
        Campuses.id.label("id"),
        Campuses.name.label("name"),
        Campuses.slug.label("slug"),
    ).filter(_filter).first()

    if campus is None:
        return None

    users_object = app.db.session.query(
        Users.id.label("id"),
    ...
    ).join(Campuses).filter(
        Users.campus_id == campus.id,
        (Users.hidden == False) if not admin else True,
        (Users.banned == False) if not admin else True,
    ).all()

    from .scores import get_standings

    standings = get_standings()
    campus_standings = [standing for standing in standings if standing.campus_id == campus.id]
